...
class PaginationProvider<T extends IModelWithId, U extends IBasePaginationRepository<
  ...
  final paginationThrottle = Throttle(
    const Duration(seconds: 3), // 얼마 동안 throttle을 할 것인지. 3초 이내에는 새로운 요청X
    initialValue: _PaginationInfo(), // 처음에 어떤 값으로 실행할 것인지.
    checkEquality: false, // 시간 내에 함수를 실행할 때, initialValue와 같은 함수면 실행 할 것인지?
  );

  PaginationProvider({
    required this.repository,
  }) : super(CursorPaginationLoading()) {
    paginate(); //RestaurantStateNotifier가 생성되는 순간에 paginate 실행.

    //paginationThrottle에 값이 생성될 때 listen.
    //즉, paginate 함수가 실행될 때 listen 함수가 실행된다.
